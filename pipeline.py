...
def load_and_preprocess():
    ...
    # 1. CLEANING: Remove spaces from column names and string values
    ...
    # CRITICAL FIX: Ensure 'Approved' is 1 and 'Rejected' is 0
...
def get_model(model_name):
    # XGBoost is usually the most accurate for this dataset
    if model_name == "XGBoost":
        return XGBClassifier(n_estimators=200, max_depth=5, learning_rate=0.1, random_state=42)
    elif model_name == "Random Forest":
        return RandomForestClassifier(n_estimators=100, random_state=42)
    else:
        # Logistic Regression (Requires scaling, but we'll stick to tree-based for now)
